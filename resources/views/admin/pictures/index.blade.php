	...
	@section('content')
		...
		<div class="row d-none d-xl-flex">
			...
			<div class="col-10">
				...
				<div class="scrollspy-example text-center p-0" data-spy="scroll" data-target="#mdb-scrollspy-ex" data-offset="0" id="">
					...
					@foreach($getLocations as $location)
						...
						<section class="currentPicturesDiv" id="item-{{ $loop->iteration }}">
							...
							@if($location->pictures->count() > 0)
								<div class="">
									<span class="text-light"><i>Total Pictures:</i> {{ $location->pictures->count() }}</span>
								</div>
							@endif
				...
				<div class="" id="mdb-scrollspy-ex">
					...
					<ul class="nav nav-pills default-pills smooth-scroll">
						...
						@foreach($getLocations as $location)
							...
							<li class="nav-item">
								<a class="nav-link" href="#item-{{ $loop->iteration }}">{{ $location->trip_location }}</a>
							</li>
